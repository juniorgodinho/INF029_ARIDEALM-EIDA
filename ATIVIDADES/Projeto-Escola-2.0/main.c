...
int relatorio(int op2){
  ...
  printf("12-Lista de pessoas a partir de, mínimo 3 letras;\n");
  printf("0-Sair;\n");
  scanf("%d", &op2);
  system("cls");
...
void aniversariantes(struct dados p[],struct dados a[], int *ptr, int *cont){//listar aniversariantes
  ...
  if(contA==0)
	...
	/*for(i=0; i<tam;i++){//tam sem o & vai indicar valor, se pasou por referencia, vai permanecer o valor que alterou na função, emtao aqui pega o valor, mesmo esquema na impressão do exemplo
